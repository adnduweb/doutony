"use strict";
var KTAppProductListDatatable = function() {
    var t;
    return {
        init: function() {
            t = $("#kt_apps_product_list_datatable").KTDatatable({
                data: {
                    type: "remote",
                    source: {
                        read: {
                            ...
                            params: {
                                ...
                                action: 'list',
                                value: '',
                                module: window.btoa('Adnduweb/Ci4_' + baseSegmentAdmin + ''),
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        template: function(t) {
                            var categorieName = '';
                            $.each(t.ec_categories, function(index, value) {
                                categorieName += value.name + ', ';
                            });
                            categorieName = categorieName.substring(0, categorieName.length - 2);
                            return '<span class="btn btn-bold btn-sm btn-font-sm btn-label-dark">' + categorieName + "</span>";
                        }
                        ...
                        <li class="navi-item">\
                        <a href="' + startUrl + '/' + baseSegmentAdmin + '/catalogue/product/edit/' + t.id + '" class="navi-link">\
                        <span class="navi-icon"><i class="flaticon2-contract"></i></span>\
                        <span class="navi-link-text">' + _LANG_.edit + '</span>\
                        </a></li>';
                            template += ' <li class="navi-item">\
                            <a href="' + startUrl + '/' + baseSegmentAdmin + '/catalogue/product/dupliquer/' + t.id + '" data-id="' + t.id + '" class="navi-link">\
                ...
                event.preventDefault();
                var e = $(this).find(".navi-link-text").html(),
                    st = $(this).find(".navi-link-text").data('status-etat'),
                    a = t.rows(".datatable-row-active").nodes().find('.checkbox-single > [type="checkbox"]').map(function(t, e) {
                        return $(e).val()
                    });
                a.length > 0 && swal.fire({
...
KTUtil.ready(function() {
    KTAppProductListDatatable.init();
